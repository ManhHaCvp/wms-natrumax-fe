...
const columns = [
    ...
    columnHelper.accessor("status", {
        header: "Trạng thái",
        cell: (info) => (
            <span className={`px-2 py-1 rounded-md text-white ${info.getValue() === "Đã thanh toán" ? "bg-green-500" : "bg-red-500"}`}>
        {info.getValue()}
      </span>
        ),
    }),
...
const ViewCommissionHistory = () => {
    ...
    useEffect(() => {
        const fetchCommissionData = async () => {
            try {
                await commissionHistoryService.getByReferrerId(id, setCommissionHistory);
            } catch (error) {
